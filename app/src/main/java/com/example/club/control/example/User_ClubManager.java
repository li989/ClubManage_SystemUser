...
//	}
	...
	public List<Beanclub_person> loadalluser_club(int id) throws BusinessException {
		...
		try{
			conn=DBUtil.getConnection();
			String sql="select c_name,user_grade,c_picture,c_userID,club_ID from user_club a,user b where a.c_user_ID=b.c_userID and  club_ID=? and club_status ='已通过'";
			java.sql.PreparedStatement pst=conn.prepareStatement(sql);
			pst.setInt(1, id);
			java.sql.ResultSet rs=pst.executeQuery();
			while(rs.next()){
	...
	public boolean getuser_club(int user_ID, int club_ID) throws BaseException {
		...
		try {
			conn = DBUtil.getConnection();
			String sql = "Select * from user_club where c_user_ID=? and club_ID=?";
			java.sql.PreparedStatement pst = conn.prepareStatement(sql);
