...
public class UserManager {
	...
	public void createuser(Beanuser user) throws BusinessException {
		Connection conn=null;
		try{
			conn=DBUtil.getConnection();
			...
			java.sql.ResultSet rs=pst.executeQuery();
			if(rs.next()) throw new BusinessException("该用户名已被注册");
			pst.close();
			rs.close();
			...
			sql="INSERT INTO user VALUES(?,?,?,NOW(),NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,?)";
			pst=conn.prepareStatement(sql);
			pst.setInt(1,max+1 );
	...
	public void modifyuser(Beanuser user) throws BusinessException {
		Connection conn=null;
		try{
			conn=DBUtil.getConnection();
			String sql="update user set c_pwd = ?,c_grade=?,c_birthday?,c_sex=?,c_name=?,c_sno=?,c_stugrade=?,c_major=?,c_mail=? where c_userID=?";
			java.sql.PreparedStatement pst=conn.prepareStatement(sql);
			...
			pst.setInt(10,user.getC_userID());
			pst.execute();
			pst.close();
		}
		catch(Exception e){
	...
	public Beanuser login(String zh,String pwd){
		...
		try{
			conn=DBUtil.getConnection();
			String sql="select c_pwd from user where c_number=? and c_grade=2";
			java.sql.PreparedStatement pst=conn.prepareStatement(sql);
			pst.setString(1,zh);
			java.sql.ResultSet rs=pst.executeQuery();
